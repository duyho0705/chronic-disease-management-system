...
public class ClinicalContextService {
    ...
    private final vn.clinic.cdm.clinical.repository.PrescriptionRepository prescriptionRepository;
    private final vn.clinic.cdm.patient.repository.PatientChronicConditionRepository chronicConditionRepository;
    private final vn.clinic.cdm.patient.repository.PatientVitalTargetRepository vitalTargetRepository;
    ...
    private void appendPatientProfile(StringBuilder sb, ClinicalConsultation consultation) {
        ...
        if (patient.getAllergies() != null && !patient.getAllergies().isBlank()) {
            sb.append("- âš ï¸ ALLERGIES: ").append(patient.getAllergies()).append("\n");
        } else {
    ...
    public vn.clinic.cdm.api.dto.report.CdmReportDto getCdmReportData(ClinicalConsultation consultation,
            String carePlan) {
        var patient = consultation.getPatient();
        var patientId = patient.getId();

        var conditions = chronicConditionRepository.findByPatientId(patientId).stream()
                .map(c -> vn.clinic.cdm.api.dto.report.CdmReportDto.ConditionInfo.builder()
        ...
        var targets = vitalTargetRepository.findByPatientId(patientId).stream()
                .map(t -> vn.clinic.cdm.api.dto.report.CdmReportDto.TargetInfo.builder()
                        .type(t.getVitalType())
        ...
        var adherence = medicationScheduleRepository.findByMedicationPrescriptionPatientId(patientId).stream()
                .limit(10)
                .map(s -> vn.clinic.cdm.api.dto.report.CdmReportDto.AdherenceInfo.builder()
                        .medicine(s.getMedication().getMedicineName())
                        .score(s.getStatus().equals("TAKEN") ? java.math.BigDecimal.valueOf(100)
                                : java.math.BigDecimal.ZERO)
                        .lastTaken(s.getTakenAt() != null ? s.getTakenAt().toString() : "ChÆ°a uá»‘ng")
                        .build())
                .toList();

        return vn.clinic.cdm.api.dto.report.CdmReportDto.builder()
    ...
    private void appendClinicalGuardrails(StringBuilder sb) {
        ...
        sb.append("- Local Guidelines: MOH Vietnam (Bá»™ Y Táº¿)\n");
